...
// On crée le menu dans le html
function displayMenus(menuItems) {
  let newMenu = menuItems.map(function (item) {
    return `
...
function displayAllCategoriesButtons() {
  ...
  const categoryBtns = divbtn.querySelectorAll(".filter-category-btn");
  // on crée un évenement pour chaque boutton
  categoryBtns.forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      const category = e.currentTarget.dataset.categoryName;

      if (category === "all") {
        displayMenus(menu);
      } else {
        menuToDisplay = filterMenuByCategory(category);

        displayMenus(menuToDisplay);
      }
    });
  });
}
